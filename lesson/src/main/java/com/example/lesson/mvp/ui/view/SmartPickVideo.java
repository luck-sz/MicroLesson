...
public class SmartPickVideo extends StandardGSYVideoPlayer {
    ...
    private List<VideoBean.DataBean.PlayerBean> mUrlList = new ArrayList<>();
...
//                    showSwitchDialog();
     ...
     * @return
     */
    public boolean setUp(List<VideoBean.DataBean.PlayerBean> url, boolean cacheWithPlay, String title) {
        mUrlList = url;
        return setUp(url.get(mSourcePosition).getVideoUrl(), cacheWithPlay, title);
    }
     ...
     * @return
     */
    public boolean setUp(List<VideoBean.DataBean.PlayerBean> url, boolean cacheWithPlay, File cachePath, String title) {
        mUrlList = url;
        return setUp(url.get(mSourcePosition).getVideoUrl(), cacheWithPlay, cachePath, title);
    }
...
//    }
    ...
    private void resolveStartChange(int position) {
        final String name = mUrlList.get(position).getTitle();
        if (mSourcePosition != position) {
            if ((mCurrentState == GSYVideoPlayer.CURRENT_STATE_PLAYING
                    || mCurrentState == GSYVideoPlayer.CURRENT_STATE_PAUSE)) {
                showLoading();
                final String url = mUrlList.get(position).getVideoUrl();
                cancelProgressTimer();
                hideAllWidget();
    ...
    private void resolveChangedResult() {
        isChanging = false;
        mTmpManager = null;
        final String name = mUrlList.get(mSourcePosition).getTitle();
        final String url = mUrlList.get(mSourcePosition).getVideoUrl();
        mTypeText = name;
        mSwitchSize.setText(name);
        resolveChangeUrl(mCache, mCachePath, url);
        hideLoading();
